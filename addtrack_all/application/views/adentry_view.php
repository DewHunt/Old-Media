  ...
  function addrow()
   {
	  ...
	  $('#rowcount').val(i);
	  var j=parseInt(i);
      if( $('#Caption'+j).val()==""){alert("Id cannot be blank!");$('#addmore').show();return false;}
...
function isNumberKey(evt)
  ...
  function validation()
    {
	if($('#BatchId').val()==''){alert('Please enter Batch');$('#BatchId').focus();return false;  }
 if($('#MediaId').val()=='0'){alert('Please enter Media');$('#MediaId').focus();return false;  }
 if($('#PublicationId').val()=='0'){alert('Please enter Publication');$('#PublicationId').focus();return false;  }
 if($('#Date').val()==''){alert('Please enter Date');$('#Date').focus();return false;  }
 if($('#ProductId').val()=='0'){alert('Please enter Product');$('#ProductId').focus();return false;  }
 if($('#Company').val()=='0'){alert('Please enter Company');$('#Company').focus();return false;  }
 if($('#Caption').val()==''){alert('Please enter Caption');$('#Caption').focus();return false;  }
 if($('#PriceId').val()=='0'){alert('Please enter Price');$('#PriceId').focus();return false;  }
 if($('#PageId').val()=='0'){alert('Please enter Page');$('#PageId').focus();return false;  }
 if($('#Cols').val()==''){alert('Please enter Columns');$('#Cols').focus();return false;  }
 if($('#Inchs').val()==''){alert('Please enter Inchs');$('#Inchs').focus();return false;  }
   ...
   <div class="span9" id="content">
			...
			<input type='text' name='Batch' value='<?=$batch?>' maxlength='50' tabindex='1' id='Batch' /> </td> </tr>	
			<tr> <td>Date</td> <td><input type='text' name='Date' value='<?=date('d/m/Y')?>' maxlength='50' tabindex='2' id='Date' /></td> </tr> 
            <td>Media</td>
              <td><?=form_dropdown('MediaId', $array ,'0'," tabindex='3', id='MediaId' class='datain_1' ");?></td>		   
		    <tr> <td>Publication</td> 
            <td id="ptype"><?=form_dropdown('PublicationId', array('0'=>"Select"),'0' ," tabindex='4', id='PublicationId'");?></td> </tr>
            <tr>
      ...
      <tr id="tr1">
        ...
        <td width="42">1</td>
      <td width="100" ><input    tabindex="4" name="ID1" id="ID1" type="text" value="" onblur="loadcap(1)" size="5"  style="width:100px" /></td>
		<td width="100" > 
		...
		<td width="70"><select   tabindex="8" name="pageno1" id="pageno1" style="width:70px">
	  ...
	  <td width="25"><?=form_dropdown("placingtype1",$pltype,0," id='placingtype1' tabindex='10' class='a' style='width:100px'  ");?></td>
        ...
        <td width="65"><input name="Cols1" id="Cols1" type="text" value=""  tabindex="12" style="width:65px" /></td>

<td width="41">X</td>
		
        <td width="67"><input name="Inchs1" id="Inchs1" type="text" value="" tabindex="13"  style="width:67px" /></td>
        
        
        
     <!-- </tr>
	  <tr id="addmore">
	    <td colspan="8" align="left"><a  style="cursor:pointer"  onclick="addrow()">Add More</a>/ <a  style="cursor:pointer" onclick="removerow()">Remove</a></td>
		<td colspan="9" style="text-align:right" ><a  style="cursor:pointer"  onclick="addrow()" >Add More /</a>  <a   style="cursor:pointer" onclick="removerow()"> Remove</a> </td>
	  </tr>-->
	  ...
	  <tr id="addmore">
	    <td colspan="8" align="left"><a  style="cursor:pointer" id="Enter" tabindex="100000000" onkeypress="addrow()">Add More</a>/ <a  style="cursor:pointer"   onclick="removerow()">Remove</a></td>
			...
			<input type="hidden" id="rowcount" name="rowcount" value="1" />						</td>
					 ...
					 <? } ?>
					 <? if($operation=='edit'){?>
					<form method="post" action="<?=base_url()?>index.php/<?=$controller?>/edit_ac/<?=$Eid?>" enctype="multipart/form-data">
					       <table class="table table-striped" width="100%">
			...
			<input type='text' name='Batch' value='<?=$BatchId?>' maxlength='50' tabindex='1' id='Batch' /> </td> </tr>	
			<tr> <td>Date</td> <td><input type='text' name='Date' value='<?=date('d/m/Y')?>' maxlength='50' tabindex='2' id='Date' /></td> </tr> 
            <td>Media</td>
              <td><?=form_dropdown('MediaId', $array ,$MediaId," tabindex='3', id='MediaId' class='datain_1' ");?></td>		   
		    <tr> <td>Publication</td> 
            <td id="ptype"><?=form_dropdown('PublicationId',$pbarray,$PublicationId," tabindex='4', id='PublicationId'");?></td> </tr>
            <tr>
  ...
  <table   id='tab'  width="50%" border="0">
   ...
   ?>
        ...
        <td width="42"><?=$i?></td>
      <td width="100" ><input tabindex="4" name="ID<?=$i?>" id="ID<?=$i?>" type="text" value="<?=$row->AD_ID; ?>" onblur="loadcap(1)" size="5"  style="width:40px" /></td>
		<td width="100" > 
	  ...
	  <td width="25"><?=form_dropdown("placingtype".$i,$pltype,$row->PositionId," id='placingtype".$i."' tabindex='10' class='a' style='width:100px'  ");?></td>
        ...
        <td width="65"><input name="Cols<?=$i?>" id="Cols<?=$i?>" type="text" value="<?=$row->Cols?>"  tabindex="12" style="width:35px" /></td>

<td width="41">X</td>
		
        <td width="67"><input name="Inchs<?=$i?>" id="Inchs<?=$i?>" type="text" value="<?=$row->Inchs?>" tabindex="13"  style="width:35px" /></td>
	...
	<? $i++;} ?>
	  <tr id="addmore">
	    <td colspan="8" align="left"><a href="#"  onclick="addrow()">Add More</a>/ <a href="#" onclick="removerow()">Remove</a></td>
		<td colspan="9" style="text-align:right" ><a href="#" onclick="addrow()" >Add More /</a>  <a href="#" onclick="removerow()"> Remove</a> </td>
	  </tr>
    </table>
			<input type="hidden" id="rowcount" name="rowcount" value="<?=$i-1?>" />						</td>
						</tr>
								   ...
								   <!------   View  Table   --------> 
									...
									<form method="post" action="<?=base_url()?>index.php/<?=$controller?>/delete/">
                                        ...
                                        <thead>
                                            <tr>
                                                <th width="2%">SL</th>
                                              <th width="2%"><input type="checkbox" name="chk_all" id="chk_all" value="checkbox" onclick="checkall()"></th> 
											  <th width="13%"> Id </th> 
