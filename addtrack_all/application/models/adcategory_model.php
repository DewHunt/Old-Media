...
class adcategory_model extends CI_model{
   ...
   function get_all_query_row_countall($search=NULL)
	   {  
			  ...
			  $str="select   * from  adcategory    where  State='1' ".$searchtext ;
    ...
    function get_all_query_row($p=NULL,$search=NULL)
	   {  
			  $limit="limit $p,10"; 
			  if($search!='_') {$searchtext=" and   Name like '%".$search."%'";}else {$searchtext="";}
			  $str=" select   * from  adcategory   where State='1' ".$searchtext.$limit;
			  $query=$this->db->query($str); 
			  return $query->result(); 
	   }
   function insert_mgt()
       {
				 ...
				 $str=$this->db->insert_string("adcategory",$data);		
				$query=$this->db->query($str);
	  ...
	  function edit_mgt()
       {
                ...
                $str=$this->db->update_string("adcategory",$data,$where);
				$query=$this->db->query($str);				
				return  $this->db->affected_rows();
	...
	function deleteInfo($Eid=NULL)
	  {
			$where="delete from adcategory where  Id=".$Eid." and State='1' ";
	...
	function get_all_info_by_id($eid=NULL)
	  {
	    $query=$this->db->get_where('adcategory',array('Id'=>$eid,'State'=>'1'));
		return $query->result();
