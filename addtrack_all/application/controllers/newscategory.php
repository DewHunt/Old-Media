...
class newscategory extends CI_Controller {
    ...
    public function index($msg=NULL,$setext=NULL,$limit=NULL)
    {
		    ...
		    $totalrow=$this->newscategory_model->get_all_query_row_countall(addslashes($search));
		    $config['base_url'] = base_url().'index.php/newscategory/index/_/'.$search.'/';
		   ...
		   $array=array(
					...
					'result' =>$this->newscategory_model->get_all_query_row($limit,addslashes($search)),
					'sl' =>$limit==0?1:$limit,
					'totalrow' =>  $totalrow,
					'operation' =>'add' ,
					'search'=>$search ,
		  ...
		  $this->load->view('newscategory_view',$array);
    }
    
	 public function add()
	   {  
			     $inser_id= $this->newscategory_model->insert_mgt();
				 if($inser_id==1)
					{
					  redirect('newscategory/index/1');
					}
				  else
					 {
					  redirect('newscategory/index/2');
	 ...
	 function edit($Eid=NULL,$msg=NULL,$setext=NULL,$limit=NULL)
	 
	    {
			...
			$querybyid=$this-> newscategory_model->get_all_info_by_id($Eid);
			  $Name='';
   ...
   $Description='';
	   ...
	   function edit_ac()
	     {
		         $inser_id= $this->newscategory_model->edit_mgt();
				 if($inser_id==1)
					{
					  redirect('newscategory/index/1');
					}
	 ...
	 function delete($Eid=NULL)
	   {
		 ...
		 if(isset($_POST['allvalue']) ){ 
		         for($i=0;$i<$_POST['allvalue'];$i++)
				    {
						   if(isset($_POST['chk_'.$i])) {  $this->newscategory_model->deleteInfo($_POST['chk_'.$i]); }
				 	}
		         }
			...
			$this->newscategory_model->deleteInfo($Eid); 
		   }
