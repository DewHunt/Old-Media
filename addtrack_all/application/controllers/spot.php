...
class Spot extends CI_Controller {
	...
	{
		parent::__construct();
	    $this->load->model('spot_model','',TRUE);
    ...
    public function index($msg=NULL,$setext=NULL,$limit=NULL)
    {
		    ...
		    $totalrow=$this->spot_model->get_all_query_row_countall(addslashes($search));
		    $config['base_url'] = base_url().'index.php/spot/index/_/'.$search.'/';
		   ...
		   $array=array(
					'msg' => $msg ,
					'title' =>'Add Spot Type',
					'result' =>$this->spot_model->get_all_query_row($limit,addslashes($search)),
					'sl' =>$limit==0?1:$limit,
					'totalrow' =>  $totalrow,
					'operation' =>'add' ,
					'search'=>$search ,
		  ...
		  $this->load->view('spot_view',$array);
    }
    
	 public function add()
	   {  
			     $inser_id= $this->spot_model->insert_mgt();
				 if($inser_id==1)
					{
					  redirect('spot/index/1');
					}
				  else
					 {
					  redirect('spot/index/2');
	 ...
	 function edit($Eid=NULL,$msg=NULL,$setext=NULL,$limit=NULL)
	 
	    {
			...
			$querybyid=$this-> spot_model->get_all_info_by_id($Eid);
			  $Name='';
   ...
   $Description='';
	   ...
	   function edit_ac()
	     {
		         $inser_id= $this->spot_model->edit_mgt();
				 if($inser_id==1)
					{
					  redirect('spot/index/1');
					}
	 ...
	 function delete($Eid=NULL)
	   {
		 ...
		 if(isset($_POST['allvalue']) ){ 
		         for($i=0;$i<$_POST['allvalue'];$i++)
				    {
						   if(isset($_POST['chk_'.$i])) {  $this->spot_model->deleteInfo($_POST['chk_'.$i]); }
				 	}
		         }
			...
			$this->spot_model->deleteInfo($Eid); 
		   }
